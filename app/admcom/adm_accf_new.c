...
int _CrScFormat(void *cr)
{
	int		format;

	format = crScFormat(cr);
	if(format > 1) format = 1;
	return format;
}

BOOL _CrScBlockIsEnable(void *cf)
{
	BOOL	enable;
	int		format;

	format = crScFormat(cr);
	if(format & 1) enable = 1;
	else	enable = 0;
	return enable;
}

void _CrScBlockSetEnable(void *cf, BOOL enable)
{
	int		format;
...
BOOL _CrScIso7816IsEnable(void *cf)
{
	BOOL	enable;
	int		format;

	format = crScFormat(cr);
	if(format & 2) enable = 1;
	else	enable = 0;
	return enable;
}

void _CrScIso7816SetEnable(void *cf, BOOL enable)
{
	int		format;

	format = crScFormat(cr);
	if(enable) format |= 2;
	else if(format == 3) format &= 1;
	crSetScFormat(cr, format);
}
...
void _AdmCfPrefChanged(void *pss, int index, void *act)
{
	void	*ps;
	char	temp[64];
	int		type, val, resultCode;

	ps = PSobjectAtIndex(pss, index);
	resultCode = PSresultCode(pss);
//printf("AdmCfPrefChanged: index=%d resultCode=%d\n", index, resultCode);
	type = PStype(ps);
	if(type == PS_TYPE_RADIO_GROUP) {
		PSsetBackResultCode(pss, RESULT_OK+1);
		val = PSradioGroupValue(ps);
		if(val == 0) {
			ps = PSobjectAtIndex(pss, index+3);	// block
			PSsetInteractionEnabled(ps, FALSE);
			PrefReloadRowAtIndex(act, index+3);
			ps = PSobjectAtIndex(pss, index+4);	// iso7816
			PSsetInteractionEnabled(ps, FALSE);
			PrefReloadRowAtIndex(act, index+4);
		} else {
			ps = PSobjectAtIndex(pss, index+3);
			PSsetInteractionEnabled(ps, TRUE);
			PrefReloadRowAtIndex(act, index+3);
			ps = PSobjectAtIndex(pss, index+4);
			PSsetInteractionEnabled(ps, TRUE);
			PrefReloadRowAtIndex(act, index+4);
		}
	} else if(type == PS_TYPE_CHECK) {
		PSsetBackResultCode(pss, RESULT_OK+1);
	}
}
...
//printf("AdmcfSc...tag=%d\n", PStag(pss));
	if(PStag(pss)) {
		cr = PScontext(pss);
		psg = PSaddRadioGroup(pss, PStitle(pss), _CrScFormat, crSetScFormat);
		cf = cfCardsAt(0);
		ps = PSaddRadio(pss, cfCardName(cf), 0, psg);
		PSsetButton(ps, 1, AdmCfRd);
		PSsetViewStyle(ps, PS_VIEW_STYLE_READONLY);
		PSsetContext(ps, cf);
		ps = PSaddRadio(pss, xmenu_card_format[6], 1, psg);
		PSsetButton(ps, 9, NULL);

		cf = cfCardsAt(1);
		temp[0] = temp[1] = temp[2] = ' ';
		strcpy(temp+3, cfCardName(cf));
		ps = PSaddCheck(pss, temp, _CrScBlockIsEnable, _CrScBlockSetEnable);
		PSsetButton(ps, 1, AdmCfRd);
		PSsetViewStyle(ps, PS_VIEW_STYLE_READONLY);
		PSsetContext(ps, cf);
		if(_CrScFormat(cr)) bVal = TRUE; else bVal = FALSE;
		PSsetInteractionEnabled(ps, bVal);
		cf = cfCardsAt(2);
		strcpy(temp+3, cfCardName(cf));
		ps = PSaddCheck(pss, temp, _CrScIso7816IsEnable, _CrScIso7816SetEnable);
		PSsetButton(ps, 1, AdmCfRd);
		PSsetViewStyle(ps, PS_VIEW_STYLE_READONLY);
		PSsetContext(ps, cf);
		if(_CrScFormat(cr)) bVal = TRUE; else bVal = FALSE;
