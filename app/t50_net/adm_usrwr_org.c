...
void AdmUsrWrOnAppMessage(GACTIVITY *act, unsigned char *msg)
{
	...
	case GM_FPM_RESPONSE:
		...
		switch(cmd) {
		case 0x71:
			rval = fpmResponseAddTemplate();
printf("fpmResponseAddTemplate()=%d\n", rval);
			if(rval) AlertResultMsg(xmenu_user[0], R_FAILED);
			else if(!(fpId & 1) && (fpFlag & 0x02)) fpmRequestAddTemplate(fpId+1, fpTmpls[1]);
...
printf("result=%d\n", result);
		...
		case P_WRITE_OBJECT:
			...
			case OT_USER:
				...
				if(result) {
					if(result == R_OBJECT_IS_FULL) index = R_USER_ARE_FULL;
					else if(result == 99) index = R_ACU_TIMEOUT;
					else 	index = R_ACU_OTHER_ERROR;
...
void *AdmUsrWrOnCellForRowAtIndex(void *self, void *tblv, int index)
{
	...
	case 1:
		...
		if(user->id > 0) sprintf(temp, "%ld", user->id);
		else	temp[0] = 0;
		UIsetText(lbl2, temp);
...
void AdmUsrWrOnDidSelectRowAtIndex(void *self, void *tblv, int index)
{
	...
	case 5:
		...
		AdmUsrCredStart(ACT_ADM_INPUT+index, 0);
		break;
	case 6:
		bcd2string(user->pin, 8, pwd);
