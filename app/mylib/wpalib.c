...
int wpaPipePoll(int pipe_fd, int timeout)
{
	struct pollfd	fds[3];
	unsigned char	buf[8];
	int		rval;
...
int wpaCtrlRequest(char *cmd, char *reply, int reply_len)
{
	struct timeval tv;
	fd_set	rfds;
	int		rval, recvlen;
	...
	if(FD_ISSET(ctrl_s, &rfds)) {
		rval = wpa_ctrl_recv(ctrl_s, reply, reply_len);
if(rval > 0) {
...
//printf("recv=%d\n", rval);
		while(rval >= 0) {
			...
			rval = select(ctrl_s + 1, &rfds, NULL, NULL, &tv);
			if(FD_ISSET(ctrl_s, &rfds)) {
				rval = wpa_ctrl_recv(ctrl_s, reply+recvlen, reply_len-recvlen);
printf("more recv=%d\n", rval);
			} else rval = -2;
		}
		if(rval != -1) {
			reply[recvlen] = 0;
if(recvlen > 256) printf("Reply: %d [.....]\n", recvlen);
else printf("Rx: %s", reply);
			rval = recvlen;
		}
	} else {
printf("Timeout...\n");
...
int wpaCtrlMoreResponse(char *reply, int reply_len)
{
	struct timeval tv;
	fd_set	rfds;
	int		rval, recvlen;
...
int wpaMonRead(char *buf, int len)
{
	int		rval;
