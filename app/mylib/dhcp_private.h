...
typedef struct _DHCP {
	...
	unsigned char	svrMacAddr[6];
	unsigned char	ipAddr[4];
	unsigned char	gwAddr[4];
	unsigned char	subnet[4];
	unsigned char	dnsServers[4][4];
	unsigned char	svrAddr[4];
	unsigned char	fromAddr[4];
...
#define INADDR_ANY_ADDR			(unsigned char *)"\x00\x00\x00\x00"
#define INADDR_BROADCAST_ADDR	(unsigned char *)"\xff\xff\xff\xff"

//
#define DHCP_FLAGSBROADCAST		0x8000

// UDP port numbers for DHCP
#define	DHCP_SERVER_PORT		(unsigned short)67		// from server to client
#define DHCP_CLIENT_PORT		(unsigned short)68		// from client to server
...
typedef struct _UDP_DHCP_PACKET {
	struct iphdr	ip;
	struct udphdr	udp;
	RIP_MSG			msg;
} UDP_DHCP_PACKET;
