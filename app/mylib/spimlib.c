...
int spimReadInterrupt(int fd, unsigned char *buf)
{
	...
	xfer = &_xfer;
//	xfer->headbuf = (unsigned long)headbuf; xfer->bodybuf = (unsigned long)buf; xfer->size = 4; xfer->read = 1;
...
int spimReadInterrupt2(int fd, unsigned char *buf)
{
	...
	headbuf[0] = 0xaa; headbuf[1] = SPI_READ_INTERRUPT << 4; headbuf[2] = 0;
	xfer = &_xfer;
	xfer->headbuf = (unsigned long)headbuf; xfer->bodybuf = (unsigned long)buf; xfer->size = 2; xfer->read = 1;
	rval = ioctl(fd, SPI_IOC_WR_XFER, xfer);
	if(rval >= 0) rval = xfer->rval;
//printf("%lu spimReadInterrupt()=%d\n", MS_TIMER, rval);
	return rval;
