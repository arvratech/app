...
// S-BOX
const static unsigned long SS0[256] = {
...
const static unsigned long SS1[256] = {
...
const static unsigned long SS2[256] = {
...
const static unsigned long SS3[256] = {
...
void SEED_Encrypt_Block(SEED_CTX *ctx, unsigned long *data)
{
	unsigned long	L0, L1, R0, R1;		// Iuput/output values at each rounds
	unsigned long	*K, T0, T1;			// Temporary variables for round function F
...
void SEED_Decrypt_Block(SEED_CTX *ctx, unsigned long *data)
{
	unsigned long	L0, L1, R0, R1;		// Iuput/output values at each rounds
	unsigned long	*K, T0, T1;			// Temporary variables for round function F
...
void SEED_Encrypt(SEED_CTX *ctx, unsigned char *in, unsigned char *out)
{
	unsigned char	*p;
	unsigned long	lVal, data[4];

	p = in;
	data[0] = p[0]  << 24 | p[1]  << 16 | p[2]  << 8 | p[3];
	data[1] = p[4]  << 24 | p[5]  << 16 | p[6]  << 8 | p[7];
	data[2] = p[8]  << 24 | p[9]  << 16 | p[10] << 8 | p[11];
	data[3] = p[12] << 24 | p[13] << 16 | p[14] << 8 | p[15];
	SEED_Encrypt_Block(ctx, data);
	p = out;
	lVal = data[0];
	p[0]  = (unsigned char)(lVal >> 24);
	p[1]  = (unsigned char)(lVal >> 16);
	p[2]  = (unsigned char)(lVal >> 8);
	p[3]  = (unsigned char)lVal;
	lVal = data[1];
	p[4]  = (unsigned char)(lVal >> 24);
	p[5]  = (unsigned char)(lVal >> 16);
	p[6]  = (unsigned char)(lVal >> 8);
	p[7]  = (unsigned char)lVal;
	lVal = data[2];
	p[8]  = (unsigned char)(lVal >> 24);
	p[9]  = (unsigned char)(lVal >> 16);
	p[10] = (unsigned char)(lVal >> 8);
	p[11] = (unsigned char)lVal;
	lVal = data[3];
	p[12] = (unsigned char)(lVal >> 24);
	p[13] = (unsigned char)(lVal >> 16);
	p[14] = (unsigned char)(lVal >> 8);
	p[15] = (unsigned char)lVal;
}

void SEED_Decrypt(SEED_CTX *ctx, unsigned char *in, unsigned char *out)
{
	unsigned char	*p;
	unsigned long	lVal, data[4];
...
void SeedInit(SEED_CTX *ctx, unsigned char *Key)
{
	...
	unsigned long	*K, T0, T1;

	memset(ctx, 0, sizeof(SEED_CTX));
	K = ctx->key_data;		// Pointer of round keys
	// Set up input values for Key Schedule	
	A = Key[0]  << 24 | Key[1]  << 16 | Key[2]  << 8 | Key[3];
	B = Key[4]  << 24 | Key[5]  << 16 | Key[6]  << 8 | Key[7];
	C = Key[8]  << 24 | Key[9]  << 16 | Key[10] << 8 | Key[11];
	D = Key[12] << 24 | Key[13] << 16 | Key[14] << 8 | Key[15];
...
void SeedEncryptECB(SEED_CTX *ctx, unsigned char *in, unsigned char *out, int len)
{
	...
	for(p1 = in, p2 = out;l > 0;l -= SEED_BLOCK, p1 += SEED_BLOCK, p2 += SEED_BLOCK) SEED_Encrypt(ctx, p1, p2);
}
...
void SeedDecryptECB(SEED_CTX *ctx, unsigned char *in, unsigned char *out, int len)
{
	...
	for(p1 = in, p2 = out;l > 0;l -= SEED_BLOCK, p1 += SEED_BLOCK, p2 += SEED_BLOCK) SEED_Decrypt(ctx, p1, p2);
}

void SeedEncryptCBC(SEED_CTX *ctx, unsigned char *IV, unsigned char *in, unsigned char *out, int len)
{
	register int	i, l = len;
	unsigned char	*p1, *p2, tmp[SEED_BLOCK];

	if(IV) memcpy(tmp, IV, SEED_BLOCK);
	else	memset(tmp, 0, SEED_BLOCK);
	for(p1 = in, p2 = out;l > 0;l -= SEED_BLOCK, p1 += SEED_BLOCK, p2 += SEED_BLOCK) {
		for(i = 0;i < SEED_BLOCK;i++) tmp[i] ^= p1[i];
		SEED_Encrypt(ctx, tmp, p2);
		memcpy(tmp, p2, SEED_BLOCK);
	}
}
...
void SeedDecryptCBC(SEED_CTX *ctx, unsigned char *IV, unsigned char *in, unsigned char *out, int len)
{
	...
	for(p1 = in, p2 = out;l > 0;l -= SEED_BLOCK, p1 += SEED_BLOCK, p2 += SEED_BLOCK) {
		SEED_Decrypt(ctx, p1, p2);
		for(i = 0;i < SEED_BLOCK;i++) p2[i] ^= tmp[i];
		memcpy(tmp, p1, SEED_BLOCK);
	}
}
...
int SeedPadding(unsigned char *data, int len, int pad_method, int pad_byte)
{
	...
	if(pad_method == 3) {
		if(len & (SEED_BLOCK-1)) {
			flen = ((len + SEED_BLOCK - 1) >> 4) << 4;
			memset(data+len, pad_byte, flen-len);
		} else {
			memset(data+len, 0x0f, SEED_BLOCK);
			flen = len + SEED_BLOCK;  
		}
	} else if(pad_method == 1) {
		if(len & (SEED_BLOCK-1)) {
			flen = ((len + SEED_BLOCK - 1) >> 4) << 4;
			memset(data+len, 0x00, flen-len);
		} else {
			data[len] = 0x80;
			memset(data+len+1, 0x00, SEED_BLOCK-1);
			flen = len + SEED_BLOCK;  
		}
	} else {
		flen = ((len + SEED_BLOCK - 1) >> 4) << 4;
		if(flen > len) memset(data+len, 0, flen-len);
	}
	return flen;
}

/*
unsigned char Key[16]	= { 0x88, 0xe3, 0x4f, 0x8f, 0x08, 0x17, 0x79, 0xf1, 0xe9, 0xf3, 0x94, 0x37, 0x0a, 0xd4, 0x05, 0x89 }; 
unsigned char IV[16]	= { 0x26, 0x8d, 0x66, 0xa7, 0x35, 0xa8, 0x1a, 0x81, 0x6f, 0xba, 0xd9, 0xfa, 0x36, 0x16, 0x25, 0x01 };
unsigned char Plain[64]	= { 0xd7, 0x6d, 0x0d, 0x18, 0x32, 0x7e, 0xc5, 0x62, 0xb1, 0x5e, 0x6b, 0xc3, 0x65, 0xac, 0x0c, 0x0f,
...
{
	SEED_CTX	*ctx, _ctx;
	unsigned char	org_data[128], enc_data[128], dec_data[128];
	int		i, len, enc_len;
	
	ctx = &_ctx;
	SeedInit(ctx, Key); 
	cprintf ("Key  : [%02x", Key[0]);
	for(i = 1;i < 16;i++) cprintf("-%02x", Key[i]); cprintf("]\n");
	cprintf ("IV   : [%02x", IV[0]);
	for(i = 1;i < 16;i++) cprintf("-%02x", IV[i]); cprintf("]\n");
	len = 16;
	cprintf("Plain(%d)   : [%02x", len, Plain[0]);
	for(i = 1;i < len;i++) if(!(i & 0x0f)) cprintf("]\n              [%02x", Plain[i]); else cprintf("-%02x", Plain[i]); cprintf("]\n");	
	memcpy(org_data, Plain, len);
	enc_len = SeedPadding(org_data, len, 1, 0);
memset(org_data+16, 16, 16);
	SeedEncryptCBC(ctx, IV, org_data, enc_data, enc_len);
	cprintf ("Encrypt(%d) : [%02x", enc_len, enc_data[0]);
	for(i = 1;i < enc_len;i++) if(!(i & 0x0f)) cprintf("]\n              [%02x", enc_data[i]); else cprintf("-%02x", enc_data[i]); cprintf("]\n");
	SeedDecryptCBC(ctx, IV, enc_data, dec_data, enc_len);
	cprintf("Decrypt(%d) : [%02x", enc_len, dec_data[0]);
	for(i = 1;i < enc_len;i++) if(!(i & 0x0f)) cprintf("]\n              [%02x", dec_data[i]); else cprintf("-%02x", dec_data[i]); cprintf("]\n");
}
*/
