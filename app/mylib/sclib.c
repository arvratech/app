...
int scGetKey(SYS_CFG *sys_cfg, unsigned char *data)
{
	...
	p = data;
	memcpy(p, sys_cfg->smartCardInfo.mifareKey, 6); p += 6;
	memset(p, 0xff, 6); p += 6;	// sys_cfg->SCSecondaryKey
	*p++ = 0x00;				// sys_cfg->SCKeyOption;
	return p - data;
}
...
int scSetKey(SYS_CFG *sys_cfg, unsigned char *data)
{
	...
	p = data;	
	memcpy(sys_cfg->smartCardInfo.mifareKey, p, 6); p += 6;
	p += 6;		// sys_cfg->SCSecondaryKey
...
{
	if(scValidateKey(data)) memcpy(sys_cfg->smartCardInfo.mifareKey, data, 6);
}

int scCompareKey(SYS_CFG *sys_cfg, unsigned char *data)
{
	if(memcmp(sys_cfg->smartCardInfo.mifareKey, data, 6)) return 1;
	else return 0;
}
