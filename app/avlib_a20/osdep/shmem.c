...
while(1){
  ...
  case 0:  // ========= MAP_ANON|MAP_SHARED ==========
    ...
    printf("shmem: %d bytes allocated using mmap anon (%p)\n", size, p);
    return p;
#else
// system does not support MAP_ANON at all (e.g. solaris 2.5.1/2.6), just fail
    printf("shmem: using mmap anon failed\n");
#endif
    break;
  case 1:  // ========= MAP_SHARED + /dev/zero ==========
    ...
    if(p==MAP_FAILED) break; // failed
    printf("shmem: %d bytes allocated using mmap /dev/zero (%p)\n", size, p);
    return p;
  case 2: { // ========= shmget() ==========
#ifdef HAVE_SHM
    ...
    if ((p = shmat(shmemid, 0, 0)) == (void *)-1){
      printf("shmem: shmat() failed: %s\n", strerror(errno));
      shmctl (shmemid, IPC_RMID, &shmemds);
      break;
    }
    if (shmctl(shmemid, IPC_RMID, &shmemds) == -1) {
      printf("shmem: shmctl() failed: %s\n", strerror(errno));
      if (shmdt(p) == -1) perror ("shmdt()");
      break;
    }
    printf("shmem: %d bytes allocated using SHM (%p)\n",size,p);
    return p;
#else
    printf("shmem: no SHM support was compiled in!\n");
    return NULL;
#endif
    }
  default:
...
void shmem_free(void* p,int size){
  switch(shmem_type){
    ...
    case 1:
	    if(munmap(p,size)) {
		printf("munmap failed on %p %d bytes: %s\n", p, size, strerror(errno));
	    }
      break;
    case 2:
#ifdef HAVE_SHM
	    if (shmdt(p) == -1)
		printf("shmfree: shmdt() failed: %s\n", strerror(errno));
#else
	    printf("shmfree: no SHM support was compiled in!\n");
#endif
      break;
  }
}
