...
mdct_info *faad_mdct_init(uint16_t N)
{
	...
	if(aachandle < 0) {
		printf("### open AAC device(/dsp/aac) ###\n");
		aachandle = open( "/dev/aac", O_RDWR);
		if(aachandle < 0) {
			printf( "### Error: cannot open device for aac decoder\n");
			return NULL;
		}
	}
//	printf("### faad_mdct_init ###\n");
    mdct->N = N;
...
#if 1
	if(aachandle >= 0) {
		printf( "### Close AAC device ###\n" );
		close(aachandle);
		aachandle = -1 ;
	}
//	printf( "### faad_mdct_end ###\n" );
#endif
}
