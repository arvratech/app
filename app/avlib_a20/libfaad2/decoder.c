...
uint8_t NEAACDECAPI NeAACDecSetConfiguration(NeAACDecHandle hDecoder, NeAACDecConfigurationPtr config)
{
    if(hDecoder && config) {
                                 ...
                                 uint32_t *samplerate, uint8_t *channels, int latm_stream)
{
    ...
    if(!hDecoder || !samplerate || !channels)
        return -1;
    hDecoder->sf_index = get_sr_index(hDecoder->config.defSampleRate);
    hDecoder->object_type = hDecoder->config.defObjectType;
    *samplerate = get_sample_rate(hDecoder->sf_index);
    *channels = 1;
printf("NeAACDecInit: buffer=0x%x size=%d\n", buffer, buffer_size);
    if(buffer) {
        int is_latm;
        ...
        if(is_latm && l->ASCbits > 0) {
            ...
            x = NeAACDecInit2(hDecoder, l->ASC, (l->ASCbits+7)/8, samplerate, channels);	// KDK
            if(x != 0) hDecoder->latm_header_present = 0;
            return x;
        } else if(latm_stream) {
            return -1;
        } else
                                 ...
                                 uint32_t *samplerate, uint8_t *channels)
{
	...
	if(!hDecoder || !pBuffer || SizeOfDecoderSpecificInfo < 2 || !samplerate || !channels)
		return -1;
printf("NeAACDecInit2: buffer=0x%x size=%d\n", pBuffer, SizeOfDecoderSpecificInfo);
	hDecoder->adif_header_present = 0;
	hDecoder->adts_header_present = 0;
...
printf("NeAACDecInit2: ot=%d sr=%d ch=%d\n", hDecoder->object_type, *samplerate, *channels);
#ifdef ERROR_RESILIENCE
...
void* NEAACDECAPI NeAACDecDecode(NeAACDecHandle hDecoder, NeAACDecFrameInfo *hInfo,
                                 uint8_t *inBuf, uint32_t inLen)
{
    return aac_frame_decode(hDecoder, hInfo, inBuf, inLen, NULL, 0);
}

void* NEAACDECAPI NeAACDecDecode2(NeAACDecHandle hDecoder, NeAACDecFrameInfo *hInfo,
                                  uint8_t *inBuf, uint32_t inLen,
                                  void **outBuf, uint32_t outLen)
{
    ...
    return aac_frame_decode(hDecoder, hInfo, inBuf, inLen, outBuf, outLen);
}
                              ...
                              void **outBuf2, uint32_t outLen)
{
    ...
    faad_initbits(&ld, inBuf, inLen);
#if 0
    {
        int i;
        for (i = 0; i < ((inLen+3)>>2); i++)
        {
            uint8_t *buf;
            uint32_t temp = 0;
    ...
    if(hDecoder->adts_header_present) {
        ...
        if((hInfo->error = adts_frame(&adts, &ld)) > 0)
            goto error;
        // MPEG2 does byte_alignment() here, but ADTS header is always
		// multiple of 8 bits in MPEG2 so not needed to actually do it.
    }
#ifdef ANALYSIS
    dbg_count = 0;
#endif
    ...
    if(hDecoder->latm_header_present) hInfo->header_type = LATM;
#if (defined(PS_DEC) || defined(DRM_PS))
    ...
    if(!hDecoder->sample_buffer || hDecoder->alloced_channels != output_channels) {
		...
		size = frame_len * output_channels * stride;
        if(outLen == 0) {
            if(hDecoder->sample_buffer) faad_free(hDecoder->sample_buffer);
            hDecoder->sample_buffer = NULL;
            hDecoder->sample_buffer = faad_malloc(size);
        } else if(outLen < size) {
...
#endif
    outBuf = output_to_PCM(hDecoder, hDecoder->time_out, outBuf,
