...
static unsigned char	cbuf[8+2048];
...
static void _OnAppMessage(uv_poll_t *handle, int status, int events)
{
	...
	if(rval >= 10) {
		...
		} else if(buf[0] == GM_ACTIVITY) {
		} else if(act->onAppMessage) {
...
//	pthread_join(pt_tinfo.thread_id, &res);
	...
	tsClose(_poll_ts.io_watcher.fd);
}
...
static void _MethodActivity(ACT_CMD *acq)
{
	int		method;

	method = acq->method; acq->method = 0;
printf("####### activity %d ########\n", method);
	switch(method) {
	case START_ACTIVITY:
		appStartActivity(acq->act, acq->intent, (int)acq->flag);
		break;
	case START_ACTIVITY_FOR_RESULT:
		appStartActivityForResult(acq->act, acq->requestCode, acq->intent, (int)acq->flag);
		break;
	case DESTROY_ACTIVITY:
		appDestroyActivity(acq->act);
		break;
	case DESTROY_ACTIVITY_AFFINITY:
		appDestroyActivityAffinity(acq->act);
		break;
	}
	acq->method = 0;
}
...
int appStartActivity(GACTIVITY *act, char *intent, int flag)
{
	...
	if(acq->pending) {
		acq->act = act;
		acq->method = START_ACTIVITY;
	}
}

int _AppStartActivity(GACTIVITY *act, char *intent, int flag)
{
	GACTIVITY	*a;
	void		*wnd;
	CGRect		rt;
	...
	if(acq->pending) {
		acq->act = act;
		acq->method = START_ACTIVITY;
		if(intent && intent[0]) strcpy(acq->intent, intent);
		else	acq->intent[0] = 0;
		if(flag) acq->flag = 1;
		else	 acq->flag = 0;
		return 0;
	...
	acq->pending = 0;
	if(acq->method) _MethodActivity(acq);
	return 0;
}
...
int appStartActivityForResult(GACTIVITY *act, int requestCode, char *intent, int flag)
{
	...
	if(acq->pending) {
		acq->act = act;
		acq->method = START_ACTIVITY_FOR_RESULT;
		acq->requestCode = requestCode;
		if(intent && intent[0]) strcpy(acq->intent, intent);
		else	acq->intent[0] = 0;
		if(flag) acq->flag = 1;
		else	 acq->flag = 0;
...
int appDestroyActivityAffinity(GACTIVITY *act)
{
	...
	if(acq->pending) {
		acq->act = act;
		acq->method = DESTROY_ACTIVITY_AFFINITY;
		return 0;
	}
	if(gActCount < 1 || gActs[gActCount-1] != act) {
		printf("UIApp: can't destroy activity for non-exist\n");
