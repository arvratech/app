...
{
	CGRect	rt, *clipRect, _clipRect;
	int		x, y, w, h;

//printf("g2dFill: %d,%d %d %d\n", lx, ly, width, height);
	rt.x = lx; rt.y = ly; rt.width = width; rt.height = height;
	if(g2d->type) clipRect = &_lcdClipRect;
	else {
		clipRect = &_clipRect; clipRect->x = clipRect->y = 0;
		clipRect->width = g2d->width; clipRect->height = g2d->height;
	}
	rectIntersection(&rt, clipRect, &rt);
	if(g2d->type && _lcdLandscape) {
		x = g2dLcd->width-rt.height-rt.y; y = rt.x; w = rt.height; h = rt.width;	// 90
	} else {
...
//printf("blit:\n");
		...
		if(g2d->type && _lcdLandscape) {
			x = g2d->width-rt.height-y; y = rt.x;
		}
		bop.dst_x				= x;
		bop.dst_y				= y;
		...
		if(g2d->type && _lcdLandscape) flag |= G2D_BLT_ROTATE270;
		bop.flag				= flag;
		bop.alpha				= alpha;
		bop.color				= 0;
...
//printf("stretch: %d %d => %d %d\n", g2dS->width, g2dS->height, w, h); 
		...
		if(g2d->type && _lcdLandscape) {
			x = g2d->width-rt.height-y; y = rt.x; 	// 90
		}
		sop.dst_rect.x			= x;
		sop.dst_rect.y			= y;
		sop.dst_rect.w			= w;
...
void g2dMove(G2D_BUFFER *g2d, CGRect *rt, int dy)
{
	int		x, y, w, h;
	...
	if(g2d->type && _lcdLandscape) {
		y = rt->x; h = rt->width;
		if(dy > 0) {
			w = rt->height - dy; x = g2d->width - rt->height - rt->y + dy;
		} else {
			w = rt->height + dy; x = g2d->width - rt->height - rt->y;
		}
	} else {
		x = rt->x; w = rt->width;
		if(dy > 0) {
			h = rt->height - dy; y = rt->y;
		} else {
			h = rt->height + dy; y = rt->y - dy;
		}
	}
