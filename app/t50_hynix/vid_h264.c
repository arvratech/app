...
static pj_status_t vh264_h264_codec_open(pjmedia_vid_codec *codec,
...
PJ_DEF(pj_status_t)pjmedia_codec_vpx_vid_init(pjmedia_vid_codec_mgr *mgr, pj_pool_factory *pf)
{
    pj_status_t status;

    if(!mgr) mgr = pjmedia_vid_codec_mgr_instance();
    PJ_ASSERT_RETURN(mgr, PJ_EINVAL);
    // Create VH264 codec factory. */
    vpx_factory.base.op = &vh264_factory_op;
    vpx_factory.base.factory_data = NULL;
    vpx_factory.mgr = mgr;
    // Register codec factory to codec manager
    status = pjmedia_vid_codec_mgr_register_factory(mgr, &vh264_factory.base);
    if (status != PJ_SUCCESS) status;
 PJ_LOG(4,(THIS_FILE, "VH264 codec initialized"));
    return PJ_SUCCESS;
}

 // Unregister VH264 codecs factory from pjmedia endpoint.
PJ_DEF(pj_status_t)pjmedia_codec_vpx_vid_deinit(void)
{
    pj_status_t status = PJ_SUCCESS;

    // Unregister H.264 codecs factory
                                    ...
                                    pjmedia_vid_codec_param *param )
{
    ...
    /* Encoded format */
    pjmedia_format_init_video(&param->enc_fmt, info->fmt_id,
    ...
    param->dec_fmtp.param[0].name = pj_str((char*)"max-fr");
    param->dec_fmtp.param[0].val = pj_str((char*)"30");
    param->dec_fmtp.param[1].name = pj_str((char*)" max-fs");
    param->dec_fmtp.param[1].val = pj_str((char*)"580");
