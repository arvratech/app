...
int taEncodeTexts(TA_CFG *cfg, unsigned char *data)
{
	...
	p = data;
	for(i = 0;i < size;i++) {
		memcpy_pad(p, cfg->TATexts[i], 20); p += 20;
	}
	return p - data;
}
...
int taDecodeTexts(TA_CFG *cfg, unsigned char *data, int nCount)
{
	...
	p = data;	
	for(i = 0;i < nCount;i++) {
		if(i < count) memcpy_chop(cfg->TATexts[i], p, 20);
		p += 20;
	}
	return p - data;
}
...
{
	return (int)ta_cfg->TAOption;
}

void taSetOption(int Option)
{
	ta_cfg->TAOption = (unsigned char)Option;
}

void taSetOptionEx(int KeyOption, int DisplayOption)
{
	ta_cfg->TAOption  = (unsigned char)((DisplayOption << 2) + KeyOption);
}

// 1:Up/Down 2:Fixed1 3:Fixed2
int taGetKeyOption(void)
{
	return (int)(ta_cfg->TAOption & TA_KEY);
}

void taSetKeyOption(int Option)
{
	ta_cfg->TAOption = (unsigned char)((ta_cfg->TAOption & ~TA_KEY) | Option);
}

// 1:Disable 2:Text 3:Icon
int taGetDisplayOption(void)
{
	return (int)((ta_cfg->TAOption & TA_DISPLAY) >> 2);
}

void taSetDisplayOption(int Option)
{
	ta_cfg->TAOption = (unsigned char)((ta_cfg->TAOption & ~TA_DISPLAY) | (Option << 2));
}

int taGetItem(int ItemNo)
{
	return (int)ta_cfg->TAItems[ItemNo];
}

void taSetItem(int ItemNo, int Value)
{
	ta_cfg->TAItems[ItemNo] = (unsigned short)Value;
}
...
void taSetItemEx(int ItemNo, int Enable, int TextIndex, int IconIndex)
{
	...
	ta_cfg->TAItems[ItemNo] = (unsigned short)c;
}

int taItemIsEnable(int ItemNo)
{
	if(ta_cfg->TAItems[ItemNo] & TA_ITEM_ENABLE) return 1;
	else	return 0;
}

void taSetItemEnable(int ItemNo, int Enable)
{
	if(Enable) ta_cfg->TAItems[ItemNo] |= TA_ITEM_ENABLE;
	else  ta_cfg->TAItems[ItemNo] &= ~TA_ITEM_ENABLE;
}

int taGetItemTextIndex(int ItemNo)
{
	return ta_cfg->TAItems[ItemNo] & TA_ITEM_TEXT;
}
...
void taSetItemTextIndex(int ItemNo, int Index)
{
	...
	c = ta_cfg->TAItems[ItemNo];
	ta_cfg->TAItems[ItemNo] = (unsigned short)((c & ~TA_ITEM_TEXT) | Index);
}

int taGetItemIconIndex(int ItemNo)
{
	return (ta_cfg->TAItems[ItemNo] & TA_ITEM_ICON) >> 7;
}
...
void taSetItemIconIndex(int ItemNo, int Index)
{
	...
	c = ta_cfg->TAItems[ItemNo];	
	ta_cfg->TAItems[ItemNo] = (unsigned short)((c & ~TA_ITEM_ICON) | (Index << 7));
}
...
char *taGetTextAt(int Index)
{
	...
	size = taGetTextSize();
	if(Index >= 0 && Index < size) p = ta_cfg->TATexts[Index];
	else	p = NULL;
	return p;
}
