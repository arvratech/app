...
static int	iso15693DefaultSendFlags;		// default flags used for iso15693SendRequest


int iso15693Open(iso15693PhyConfig_t *config)
{
	iso15693DefaultSendFlags = 0;
...
int iso15693InventoryPresence(iso15693Card_t *card)
{
	...
	buf[0] = iso15693DefaultSendFlags | ISO15693_REQ_FLAG_INVENTORY | ISO15693_REQ_FLAG_1_SLOT;
...
printf("len=%d [%02x", len, (int)buf[0]); for(colBits = 1;colBits < len;colBits++) printf("-%02x", (int)buf[colBits]); printf("\n");
		if(len >= 10) {
			card->flags = buf[0]; card->dsfid = buf[1]; memcpy(card->uid, buf+2, 8);
...
printf("len=%d [%02x", len, (int)buf[0]); for(val = 1;val < len;val++) printf("-%02x", (int)buf[val]); printf("\n");
		...
		if(colBits >= maskBitLen) {
			...
			memcpy(mask, buf+2, len);
			val = maskBitLen & 7;
			if(val)  {
				mask[len-1] &= 0xff >> (8 - val);
			} else {
				mask[len] = 0; len++;
			}
			maskBitLen++;
...
static int _Inventory16Slot(iso15693Card_t *cards, int *cardCount, int maxCards)
{
	...
	buf[0] = iso15693DefaultSendFlags | ISO15693_REQ_FLAG_INVENTORY;
	buf[1] = ISO15693_CMD_INVENTORY;
	buf[2] = 0;
	memset(slots, 0, 16);
	rval = iso15693PhyTransceiveInventoryFrame(buf, 3, buf, &len, &colBits);
...
//printf("StayQuiet UID=[%02x", (int)uid[0]); for(len = 1;len < 8;len++) printf("-%02x", (int)uid[len]); printf("\n");
		buf[0] = iso15693DefaultSendFlags | ISO15693_REQ_FLAG_ADDRESS;
		memcpy(buf+2, uid, ISO15693_UID_LENGTH);
...
int iso15693ResetToReady(unsigned char *uid)
{
	...
	rval = iso15693Tranceive(ISO15693_CMD_RESET_TO_READY, iso15693DefaultSendFlags, uid, NULL, 0, buf, 4, &len);
    return rval;
}
...
int iso15693SelectPicc(iso15693Card_t *card)
{
	...
	rval = iso15693Tranceive(ISO15693_CMD_SELECT, iso15693DefaultSendFlags, card->uid, NULL, 0, buf, 4, &len);
    return rval;
}
...
int iso15693GetPiccSystemInformation(iso15693Card_t *card, iso15693PiccSystemInformation_t *sysInfo)
{
	...
	rval = iso15693Tranceive(ISO15693_CMD_GET_SYSTEM_INFORMATION, iso15693DefaultSendFlags, card->uid, NULL, 0, buf, 15, &len);
	if(rval) return rval;
	sysInfo->dsfid = 0;
	sysInfo->afi = 0;
...
int iso15693ReadSingleBlock(iso15693Card_t *card, int blockNo, int blockSize, iso15693PiccBlock_t *block)
{
	...
	tmpbuf[0] = blockNo;
	rval = iso15693Tranceive(ISO15693_CMD_READ_SINGLE_BLOCK, iso15693DefaultSendFlags, card->uid, tmpbuf, 1, buf, ISO15693_BUFFER_SIZE, &len);
	if(rval) return rval;
	rval = ERR_PROTOCOL;
	if(len >= 2) {
		...
		if(block->flags & ISO15693_RESP_FLAG_ERROR) {
			...
			block->errorCode = 0;
			memcpy(block->data, buf+1, blockSize);
			rval = 0;
		}
	}
	return rval;
...
int iso15693ReadMultipleBlocks(iso15693Card_t *card, int blockNo, int blockSize, int blockCount, iso15693PiccMultipleBlock_t *block)
{
	...
	rval = iso15693Tranceive(ISO15693_CMD_READ_MULTIPLE_BLOCKS, iso15693DefaultSendFlags, card->uid, tmpbuf, 2, buf, ISO15693_BUFFER_SIZE, &len);
	if(rval) return rval;
	rval = ERR_PROTOCOL;
	if(len >= 2) {
		block->flags = buf[0]; len--;
...
{
	unsigned char	*p, tmpbuf[36], buf[ISO15693_BUFFER_SIZE];
	int		rval, bitLen, colBits;

	if(!(flags & ISO15693_REQ_FLAG_OPTION)) {
//		iso15693PhySetNoResponseTime_64fcs(4238); /* ~ 20ms */
	}
	tmpbuf[0] = blockNo; memcpy(tmpbuf+1, block->data, blockSize);
