...
int  _HynixSectorCard(void);
int  _SkSamTask(void);
#endif
unsigned char	atqa[2];

int  _Iso14443_4Task(int type);
...
{
	unsigned char	*p, buf[280], buf2[16];
    int     rval, i, len;
...
#else
		if(scEmv) rval = ISO14443A_CMD_WUPA;
		else	  rval = ISO14443A_CMD_REQA;
#endif
...
printf("ACT UID=[%02x", (int)uid[1]); for(rval = 1;rval < uid[0];rval++) printf("-%02x", uid[rval+1]); printf("]\n");
				...
				if(scFindUid(uid)) {
					_scaHalt();
				} else if(scReaderFormat == 0) {
					// prohibit for mobile phone(atqa=04-00)
					//if(atqa[1] == 0x00) scAddData(0, uid+1, (int)uid[0]);
					scAddUid(uid);
	...
	case SC_READY:
#ifdef _GOV_CARD
		if(scEmv > 0 || (scEmv == 0 && (uid[1] == 0x08 || (sak & COMPLIANT_WITH_ISO14443_4)))) {
...
//printf("mifareRead: %d key=%02x-%02x-%02x-%02x-%02x-%02x\n", CrBlockNumber(), (int)p[0], (int)p[1], (int)p[2], (int)p[3], (int)p[4], (int)p[5]);
				...
				if(rval == 0) {
					memcpy(buf2, buf+9, 5); buf2[5] = 0; i = atoi((char *)buf2);
					memcpy(buf2, buf, 9); sprintf(buf2+9, "%05d", i+1); buf2[14] = buf2[15] = 0xff;
					rval = mifareWrite(CrBlockNumber(), buf2, len, uid, p);
				}
				if(rval) {
...
//		if(scCardApp == 2) rval = _SkSamTask();
		rval = _Iso14443_4Task((int)scEmv);
		if(rval >= 0) {
			buf[0] = uid[0];
			_scaHalt();
			if(rval == 0) {
